...
So as a first step, you have to assign a list of all the packages you have to check to an object. Let's say I tell you that today we will be using the following packaes: "sp",  "classInt", "RColorBrewer", "ggplot2", "hexbin", "ggmap", "XML", and "dplyr". Then you can add these to an object called `libs`, using the `c()` function: 

```{r}
libs <- c("sp",  "classInt", "RColorBrewer", "ggplot2", "hexbin", "ggmap", "XML", "dplyr")
```
...
w_nb <- poly2nb(ncovr, row.names=ncovr_sp$FIPSNO)
...
ggplot(sf_mcr_asb, aes(lng, lat)) +                        #define data and variables for x and y axes
  stat_binhex(col = "black", lwd = 0.1) +                                                         #add binhex layer (hexbin)
...
ggplot(sf_mcr_asb, aes(x = lng, y = lat)) +
  annotation_map_tile() + 
  stat_binhex(alpha=0.7, col = "black", lwd = 0.1) +                                                #add binhex layer (hexbin)
  scale_fill_gradientn(colours = c("white","red"), name = "Frequency") +     #add shading based on number of ASB incidents
  theme_void()
```
...
ggplot(sf_mcr_asb, aes(x = lng, y = lat)) + 
  annotation_map_tile() + 
  stat_bin2d(alpha=0.7, col = "black", lwd = 0.1) + 
...
ggplot(sf_mcr_asb, aes(x = lng, y = lat)) + 
  annotation_map_tile() + 
  stat_binhex(alpha=0.7, col = "black", lwd = 0.1) + 
  scale_fill_gradientn(colours = c("white","red"), 
                       name = "Frequency") + 
  theme_void()
```
...
ggplot(sf_mcr_asb, aes(x = lng, y = lat)) + 
  annotation_map_tile() + 
  stat_density2d(aes(fill = after_stat(level), # value corresponding to discretized density estimates 
...
You can, in a bind, get these data from Canvas too.
...
ggplot() +
  ...
  scale_fill_viridis_c(name = "Percent who voted to leave the EU") +
  theme_void()
```
...
Within that there is the `cartogram_cont()` function, which takes 2 arguments, 1 - the shape file (it can be a SpatialPolygonDataFrame or an sf object), and 2 - the variable which it should use to distort the polygon by. 
...
This week we learned how to improve thematic mapping choices when standard choropleths and point maps can mislead or overwhelm. We looked closely at why mapping raw rates can produce noisy "hot" and "cold" areas when denominators vary, and practised stabilising rates using empirical Bayes smoothing (both global and local) to reduce small-sample distortion while recognising the trade-offs of mapping adjusted estimates. We then explored spatial binning as an alternative to plotting thousands of points, comparing rectangular bins, hexbins, and density surfaces, and thinking through how each design choice can shift the story you tell (and the operational decisions you might make). Finally, we introduced MAUP and related edge and zoning issues to emphasise that patterns can change with the unit of aggregation, and we experimented with polygon transformations like cartograms to communicate data more legibly without pretending the map is a neutral lens. Taken together, you now have a stronger toolkit for producing clearer, fairer maps that support practical decisions, such as targeting antisocial behaviour interventions, while being explicit about the limits and assumptions built into your spatial units and visual encodings.

As this is a large area and we just gave little tasters, if you would like to learn about this more, I have included more resources to check out below: (all completely optional)
