...
This week we present a very brief and high-level overview of regression analysis, in order to give some reference to when we introduce spatial regression next week. In social science one of our main concerns is to develop models of the world, models that help us to understand the world a bit better or to predict how things will develop. 


You can read a little more about modelling in scientific research in this interactive page [here](https://www.visionlearning.com/en/library/Process-of-Science/49/Modeling-in-Scientific-Research/153). Statistics provides a set of tools that help researchers build and test scientific models.
...
# a ggplot where there are two texts one says "Unemployment" and the other "Violent crime" and there is an arrow pointing from Unemployment to Violent crime, nothing else (theme_void())
library(ggplot2)
ggplot() +
  annotate("text", x = 0.25, y = 0.6, label = "Unemployment") +
  annotate("text", x = 0.75, y = 0.6, label = "Violent crime") +
  annotate("segment", x = 0.35, xend = 0.65, y = 0.6, yend = 0.6,
           arrow = arrow()) +
  xlim(c(0,1)) +
  theme_void()
...
ggplot() +
  ...
  annotate("text", x = 0.25, y = 0.5, label = "Police levels") +
  annotate("segment", x = 0.35, xend = 0.65, y = 0.5, yend = 0.6, 
           arrow = arrow(angle = 10, type = "closed")) +
  annotate("text", x = 0.25, y = 0.7, label = "Education") +
  annotate("segment", x = 0.35, xend = 0.65, y = 0.7, yend = 0.6, 
           arrow = arrow(angle = 10, type = "closed")) +
  xlim(c(0,1)) +
  ylim(c(0,1)) + 
  theme_void()
...
```
Here we will return to the "ncovr" data set produced as part of the National Consortium on Violence Research (NCOVR) agenda (we use this also in next week, for more information on this data see @Baller_2001). The data set includes variables related to homicides in the US, as well as information in a number of sociodemographic variables that are often thought of as associated with the geographical distribution of homicides. This data set is available as part of the `geodaData` package. This means you don't need to load it from your machine, bue instead you load it with the `data()` command after having loaded the `geodaData` package. 
...
We will also be using the following packages today: 
...
The NCOVR dataset contains information about `r nrow(ncovr)` counties in the United States and if you view it you will see it has information about several decades: the 60s, 70s, 80s, and 90s. The number at the end of the variable names denotes the relevant decade, and you will see that for each decade we have the same variables.
...
### Activity 1: Familiarise yourself with the data

The first step in any analysis is to develop some familiarity with the data you are going to be working with. We have been here before. We can read the codebook if we download the data. For data inclided with an r package you can find the codebook by looking at the help file for the data set:
...
To see the specific names given to the columns you might use the names() function
...
We might want to run summary statistics for your quantitative variables, frequency distributions for your categorical variables, and visualise your variables. This will help you to detect any anomalies and give you a sense for what you have. 

If, for example, you run a histogram for the homicide rate for 1990 (HR90), you will get a sense of the distribution form, which of course is skewed.
...
stat_summary(data=ncovr, aes(x = round(RD90/0.12)*0.12, y = HR90), # Can you guess why we do not write 'RD90' as the x variable? Try it!
          ...
          size=1,
          geom = 'line') +
  annotate("segment", x=4, xend = 4, y = 45, yend= 23, color = "blue", size = 2, arrow = arrow( type = "closed", angle = 10)) +
  annotate("text", x = 4, y = 50, label = "Pick this one!", size =7, colour = "blue", 
          )+
  theme_bw()+ 
  xlab("Resource deprivation") + 
  ylab("Homicide rate")
...
As De Veaux et al (2012: 179) highlight: 
> "like all models of the real world, the line will be wrong, wrong in the sense that it can't match reality exactly. But it can help us understand how the variables are associated". 
...
### Activity 3: Regressing homicide rate on deprivation score
...
ggplot(data = mtcars[1:5, ], aes(x = mpg, y = wt)) +
  ...
  geom_label(aes(x = 21.4, y = 2.45, label = "Predicted value"), col =
               "blue") +
  geom_segment(
    aes(
      x = 22.4,
...
As then we can take this value as a measure of the strength of our model. If you look at the R output, you will see that the R2 for our model was `r summary(fit_1)$r.squared` (look at the multiple R square value in the output). We can say that our model explains `r round(summary(fit_1)$r.squared*100,2)`% of the variance in the fear of homicide. When doing regression, you will often find that regression models with aggregate data such as county level data will give you better results than when dealing with individuals. It is much harder understanding individual variation than county level variation.

As an aside, and to continue emphasising your appreciation of the object oriented nature of R, when we run the `summary()` function, we are simply generating a list object of the class summary.lm. You can extract information about your model from it. Like the R square in this case: 
...
### Activity 4: Residuals for our homicide rate model

Now then we can take this value as a measure of the strength of our model. If you look at the R output you will see that the $R^2$ for our model was `r summary(fit_1)$r.squared`. As mentioned already, we can use this to say that our model explains`r round(summary(fit_1)$r.squared*100,2)`%  of the variance in homicide. 
...
### Activity 5: Homicide rates in the South v the North
...
# first recode so it is clear that the variable 'SOUTH' is categorical
# where 0 = North and 1 = South
ncovr$SOUTH_labelled <- factor(ncovr$SOUTH, 
                         labels = c("North", "South"))
# fit the model

fit_2 <- lm(HR90 ~ as.factor(SOUTH_labelled), data=ncovr)
```
...
ggplot(data=ncovr, aes(x=SOUTH_labelled, y=HR90)) +
  geom_boxplot() +
  ...
  theme_bw() + 
  xlab("Region") + 
  ylab("Homicide rate in the 1990s")
```
...
As you will see the output does not look too different. But notice that in the print out you see how the row with the coefficient and other values for our input variable `SOUTH` we see that R is printing `as.factor(SOUTH_labelled)South`. What does this mean?
...
The reference category is the one for which R does not print the *level* next to the name of the variable for which it gives you the regression coefficient. Here we see that the named level is "South". That's telling you that the reference category here is "North". (We recoded this earlier, but if you skip this step you can just  look at the codebook you will see that 1 means the county is in a Southern state). Therefore the Y intercept in this case is the mean value of fear of violent crime for the northern counties, whereas the coefficient for the slope is telling you how much higher (since it is a positive value) the mean value is for the southern counties. Don't believe me?
...
So we have seen how we can fit models with just one predictor. We can build better models by expanding the number of predictors (although keep in mind you should also aim to build models as parsimonious (i.e. as simply) as possible).

Another reason why it is important to think about additional variables in your model is to control for spurious correlations (although here you may also want to use your common sense when selecting your variables!). You must have heard before that correlation does not equal causation. Just because two things are associated we cannot assume that one is the cause of the other. Typically we see how the pilots switch the secure the seatbelt button when there is turbulence. These two things are associated, they tend to come together. But the pilots are not causing the turbulence by pressing a switch! The world is full of **spurious correlations**, associations between two variables that should not be taken too seriously. You can explore a few [here](https://tylervigen.com/spurious-correlations). It's funny. 

Looking only at covariation between a pair of variables can be misleading. It may lead you to conclude that a relationship is more important than it really is. This is no trivial matter, it's one of the most important ones we confront in research and policy. 
...
### Activity 6: Better explaining homicide rates
...
fit_3 <- lm(HR90 ~ RD90 + SOUTH_labelled, data=ncovr)
summary(fit_3)
```
...
### Activity 7: Plotting your results
...
```{r}
fit_4 <- lm(HR90 ~ RD90 + SOUTH_labelled + DV90 + MA90 + PS90, data=ncovr)
plot_model(fit_4)
```
...
### Activity 8: Interaction effects

How do we do this in R? One way is to use the following notation in the formula argument. Notice how we have added a third term `RD90:SOUTH_labelled`, which is asking R to test the conditional hypothesis that resource deprivation may have a different impact on homicide for southern and northern counties.

```{r}
fit_5 <- lm(HR90 ~ RD90 + SOUTH_labelled + RD90:SOUTH_labelled , data=ncovr)
# which is equivalent to: 
# fit_5 <- lm(HR90 ~ RD90 * SOUTH_labelled , data=ncovr)
summary(fit_5)
```
...
+ The coefficient of *SOUTH_labelledSOUTH* can now be thought of as the difference between the predicted score of homicide rate for northern counties *that have a score of 0 in resource deprivation* and northern counties *that have a score of 0 in resource deprivation*.
...
In this week we have introduced you to multiple regression analysis. We have seen how to fit models with more than one predictor, how to interpret the results, and how to visualise them. We have also discussed how to include interaction effects in your models to test conditional hypothesis. Finally, we have briefly discussed the assumptions of regression and how to check for them. Next week we will continue our discussion of regression analysis by introducing you to spatial regression models.
